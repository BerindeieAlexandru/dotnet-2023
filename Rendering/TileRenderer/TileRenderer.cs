...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Highway && MapFeature.HighwayTypes.Any(v => p.Value.StartsWith(v))))
        {
        ...
        else if (feature.Properties.Any(p => p.Key.ToString().StartsWith(MapFeatureData.Transformer.Water.ToString())) && feature.Type != GeometryType.Point)
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Railway))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Natural && featureType == GeometryType.Polygon))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Boundary && p.Value.StartsWith("forest")))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Landuse && (p.Value.StartsWith("forest") || p.Value.StartsWith("orchard"))))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
            baseShape = geoFeature;
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == MapFeatureData.Transformer.Landuse && (p.Value.StartsWith("residential") || p.Value.StartsWith("cemetery") || p.Value.StartsWith("industrial") || p.Value.StartsWith("commercial") ||
                                                        ...
                                                        p.Value.StartsWith("brownfield"))))
        {
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == MapFeatureData.Transformer.Landuse && (p.Value.StartsWith("farm") || p.Value.StartsWith("meadow") || p.Value.StartsWith("grass") || p.Value.StartsWith("greenfield") ||
                                                        p.Value.StartsWith("recreation_ground") || p.Value.StartsWith("winter_sports") || p.Value.StartsWith("allotments"))))
        {
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
             ...
             feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Landuse && (p.Value.StartsWith("reservoir") || p.Value.StartsWith("basin"))))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Building))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Leisure))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.Transformer.Amenity))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
